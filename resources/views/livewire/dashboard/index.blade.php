<div class="max-w-6xl mx-auto mt-10 px-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="bg-white dark:bg-neutral-900 rounded-xl p-6 shadow">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm font-semibold text-gray-500 dark:text-neutral-400">Contactos recibidos
                    hoy</span>
            ...
            @can('gestionar contactos')
                <a href="{{ route('dashboard.contacts.index') }}"
                    class="mt-3 inline-block text-sm text-blue-700 hover:underline">Ver contactos</a>
            @endcan
        </div>
            ...
            <ul class="mt-2">
                @forelse ($expiringProps as $prop)
                    <li class="flex justify-between items-center py-1">
                        <span>
                            <a href="{{ route('dashboard.properties.edit', $prop->uuid) }}"
                                class="text-blue-700 hover:underline">{{ $prop->title }}</a>
                        </span>
    ...
    <div class="bg-white dark:bg-neutral-900 rounded-xl p-6 shadow mb-8">
        <span class="text-lg font-semibold text-gray-700 dark:text-neutral-200 mb-3 block">Últimos contactos
            recibidos</span>
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
            <thead>
                <tr>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-neutral-400">Nombre
                    </th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-neutral-400">Email
                    </th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-neutral-400">
                        Propiedad</th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-neutral-400">Fecha
                    </th>
            ...
            <tbody>
                @forelse ($lastContacts as $contact)
                    <tr class="hover:bg-blue-50 dark:hover:bg-blue-900/30">
                        ...
                        <td class="px-4 py-2 text-right">
                            @can('gestionar contactos')
                                <a href="{{ route('dashboard.contacts.show', $contact->uuid) }}"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-2 text-center text-gray-400">No hay contactos recientes.
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        ...
        <div class="bg-white dark:bg-neutral-900 rounded-xl p-6 shadow">
            <span class="text-lg font-semibold text-gray-700 dark:text-neutral-200 mb-3 block">Top 5 más
                visitadas</span>
            <ul class="space-y-2">
                @forelse($topVisited as $property)
                    <li class="flex justify-between items-center">
                        @can('gestionar propiedades')
                            <a href="{{ route('dashboard.properties.edit', $property->uuid) }}"
                        ...
                        @endcan
                        <span class="text-sm text-gray-500 dark:text-neutral-400">{{ $property->visits_count }}
                            visitas</span>
                    </li>
                @empty
            ...
            <ul class="space-y-2">
                @forelse($topFavorited as $property)
                    <li class="flex justify-between items-center">
                        ...
                        <span class="text-sm text-gray-500 dark:text-neutral-400">{{ $property->favorites_count }}
                            favoritos</span>
                    </li>
                @empty
